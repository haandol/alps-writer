...
class SaveHandler:
    ...
    async def handle_save_command(self, message: cl.Message, recent_history: List[BaseMessage]) -> None:
        """
        ...
        async with cl.Step(name='Generating document', type="tool") as step:
            try:
                ...
                for section_group in self.section_groups:
                    ...
                    async with cl.Step(name=group_name, type="tool") as each_step:
                        ...
                        try:
                            ...
                            logger.error(
                                "Error on streaming LLM response", error=e)
                ...
                async with cl.Step(name="Save document", type="tool"):
                    ...
                    final_document = self._combine_document_sections(
                        document_sections)

                    # Save the document to a file
                    file_path = await self._save_document_to_file(final_document, locale)

                    # Create a downloadable file
                    file = cl.File(name=os.path.basename(
